...
int main(int argc, char **argv) {
  if (argc != 7 || std::string(argv[3]) != "--seed" || std::string(argv[5]) != "--random-rays") {
    std::cerr << "usage: tetcage_oracle_report <asset.tetcage> <output.json> "
                 "--seed <u64> --random-rays <u32>\n";
    return EXIT_FAILURE;
  }
  std::uint64_t seed = 0;
  std::uint64_t random_ray_count = 0;
  if (!parse_u64(argv[4], seed) || !parse_u64(argv[6], random_ray_count) ||
  ...
  const auto rays = tetcage::generate_adversarial_rays(
      asset, asset.cage.vertices, seed, static_cast<std::uint32_t>(random_ray_count));
  const auto comparison = tetcage::compare_oracles(asset, bvh, asset.cage.vertices, rays);
  const auto interval = trace_projection(asset, bvh, rays, tetcage::ProjectionMode::interval_sum);
  const auto exact = trace_projection(asset, bvh, rays, tetcage::ProjectionMode::bounded_simplex);
