...
def _use_transparent_diffuse_probe_material() -> None:
    material = bpy.data.materials.new("TetCage_Native_Probe_TransparentDiffuse")
    material.use_nodes = True
    nodes = material.node_tree.nodes
    links = material.node_tree.links
    nodes.clear()
...
def _use_texture_probe_material() -> None:
    material = bpy.data.materials.new("TetCage_Native_Probe_Texture")
    material.use_nodes = True
    nodes = material.node_tree.nodes
    links = material.node_tree.links
    nodes.clear()
    ...
    checker.inputs["Color1"].default_value = (0.8, 0.05, 0.02, 1.0)
    checker.inputs["Color2"].default_value = (0.02, 0.1, 0.9, 1.0)
    checker.inputs["Scale"].default_value = 3.0
    emission = nodes.new("ShaderNodeEmission")
    emission.inputs["Strength"].default_value = 1.0
...
def _use_source_normal_probe_material() -> None:
    material = bpy.data.materials.new("TetCage_Native_Probe_SourceNormal")
    material.use_nodes = True
    nodes = material.node_tree.nodes
    links = material.node_tree.links
    nodes.clear()
    ...
    attribute.attribute_name = "tetcage_source_normal"
    emission = nodes.new("ShaderNodeEmission")
    links.new(attribute.outputs["Vector"], emission.inputs["Color"])
    links.new(emission.outputs["Emission"], output.inputs["Surface"])
    for obj in bpy.context.scene.objects:
        if obj.type == "MESH" and obj.get("tetcage_native_candidate"):
            obj.data.materials.clear()
...
def _use_integer_attribute_probe_material(attribute_name: str) -> None:
    material = bpy.data.materials.new(f"TetCage_Native_Probe_{attribute_name}")
    material.use_nodes = True
    nodes = material.node_tree.nodes
    links = material.node_tree.links
    nodes.clear()
    ...
    emission = nodes.new("ShaderNodeEmission")
    links.new(attribute.outputs["Fac"], emission.inputs["Color"])
    links.new(emission.outputs["Emission"], output.inputs["Surface"])
    for obj in bpy.context.scene.objects:
        if obj.type == "MESH" and obj.get("tetcage_native_candidate"):
            obj.data.materials.clear()
...
def main() -> int:
    ...
    if len(args) not in {3, 4}:
        raise RuntimeError(
            ...
            "-- asset output.exr result.json [tet_only|mixed|motion|mixed_motion|deformation_motion|deformation_local_motion|transparent|transparent_pure|transparent_diffuse_motion|transparent_motion|transparent_pure_motion|local|ao|normal|normal_transform|position|uv|texture|source_normal|source_primitive|owner_tet|material_attribute|diffuse|material|volume|volume_motion]"
        )
    ...
    if probe_mode not in {
        ...
        raise RuntimeError(
            "probe mode must be tet_only, mixed, motion, mixed_motion, deformation_motion, deformation_local_motion, transparent, transparent_pure, transparent_diffuse_motion, transparent_motion, transparent_pure_motion, local, ao, normal, normal_transform, position, uv, texture, source_normal, source_primitive, owner_tet, material_attribute, diffuse, material, volume, or volume_motion"
        )
    ...
    elif probe_mode == "transparent_diffuse_motion":
        _use_transparent_diffuse_probe_material()
    ...
    elif probe_mode == "normal_transform":
        _use_normal_probe_material()
    elif probe_mode == "position":
        _use_position_probe_material()
    elif probe_mode == "uv":
        _use_uv_probe_material()
    elif probe_mode == "texture":
        _use_texture_probe_material()
    elif probe_mode == "source_normal":
        _use_source_normal_probe_material()
    elif probe_mode == "source_primitive":
        _use_integer_attribute_probe_material("tetcage_source_primitive")
    elif probe_mode == "owner_tet":
        _use_integer_attribute_probe_material("tetcage_owner_tet")
    elif probe_mode == "material_attribute":
        _use_integer_attribute_probe_material("tetcage_material")
    elif probe_mode == "diffuse":
        _use_diffuse_probe_material()
    elif probe_mode == "material":
        _use_material_probe_material()
    elif probe_mode in {"volume", "volume_motion"}:
        _use_volume_probe_material()
    else:
        _use_probe_emission_material(mixed_scene)
    _use_black_world(scene)
    if probe_mode in {"motion", "mixed_motion", "transparent_diffuse_motion", "transparent_motion", "transparent_pure_motion", "volume_motion"}:
    ...
    scene.cycles.max_bounces = 4 if probe_mode in {"transparent", "transparent_pure", "transparent_diffuse_motion", "transparent_motion", "transparent_pure_motion", "local", "deformation_local_motion"} else 1
    scene.cycles.diffuse_bounces = 0
    scene.cycles.glossy_bounces = 0
    scene.cycles.transmission_bounces = 0
    scene.cycles.volume_bounces = 1 if probe_mode in {"volume", "volume_motion"} else 0
