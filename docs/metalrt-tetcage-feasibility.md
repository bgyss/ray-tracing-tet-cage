...
transport contract (`$CYCLES_NATIVE_WORKTREE`, revision `643723d1ee1e`, based on
...
normal/position/UV/source-normal and source material-slot handoff and point/sun lighting controls match, while the native AABB volume-table path runs; pure-transparent motion is native, while mixed Transparent+Emission motion explicitly retains ordinary Cycles fallback until its closure semantics are qualified; area-light
...
Blender branch (`c5b2d220da62`) compiles
...
motion differential passes pixel-for-pixel; an animated Blender shape-key fixture
also passes a native motion-AABB/time-interpolated differential. Static local
scanning now uses the same projected-axis narrow phase and Cycles hit-record policy as camera/shadow rays; shadow callback seams compile
...
motion, transparent, AO-local, and native shape-key deformation probes also pass.
SSS multi-hit ordering, exact volume precision, broader deformation topology, and
full Principled shading semantics remain open.
...
| Blender source | `$BLENDER_SOURCE_ROOT` at `4a09c19bea7bd2800d85f018280b2dc62e654e51` (2026-08-09), clean when inspected. The isolated candidate branch at `c5b2d220da620e2a4f042cb3b5ef4d0829d8a35b` compiles and verifies opt-in static, object-transform-motion, native shape-key deformation, AABB volume-table, static local-table, motion-local-table, and pure-transparent motion tet-cage dispatch without changing the source checkout. |
