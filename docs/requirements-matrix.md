...
| Production release | fuzzing, CI, migration, samples, reproducibility report | every supported claim linked to exact manifest | portable CI, parser/fuzz/golden foundation, and deterministic result-summary generation implemented; migration, hardware conformance, samples, and retained-backend/integration gates remain |
