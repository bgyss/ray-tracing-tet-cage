...
| Cycles | canonical loader, MetalRT and OptiX device integration | pinned source build and same-scene device comparison | static native Cycles branch `ccec7f0c3085` builds a tet-cage AABB/query path with object visibility filtering, mixed triangle/AABB candidates, compiled self-filtered opaque-shadow/local/volume seams, and Blender branch `15e7cfa2b08b` builds the full app, recognizes `cycles_tetcage_v1`, and passes opt-in tet-only and mixed ordinary-triangle/native-AABB emission differentials; motion, record-all/local-runtime/volume-runtime/transparent/Principled shading semantics, M9, and NVIDIA/OptiX remain gates |
