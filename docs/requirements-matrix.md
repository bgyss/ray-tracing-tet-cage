...
| Cycles | canonical loader, MetalRT and OptiX device integration | pinned source build and same-scene device comparison | static native Cycles branch `018653fbf` builds a tet-cage AABB BLAS/query path and matches the ordinary triangle emission reference on Apple M1 Max; source provenance is packed into `ShaderData`; Blender fallback records a `cycles_tetcage_v1` native candidate; motion, full visibility/shading semantics, Blender-native sync, M9, and NVIDIA/OptiX remain gates |
