...
| Cycles | canonical loader, MetalRT and OptiX device integration | pinned source build and same-scene device comparison | static native Cycles branch `bb9508937` builds a tet-cage AABB BLAS/query path with object visibility filtering, a compiled self-filtered opaque-shadow seam, source provenance packed into `ShaderData`, and an emission reference match on Apple M1 Max; Blender branch `d4d3ec46c93c` builds the full app, recognizes `cycles_tetcage_v1`, and passes an opt-in static tet-only native-vs-fallback emission differential with a native AABB BLAS; mixed ordinary geometry, motion, record-all/local/volume/transparent/full shading semantics, M9, and NVIDIA/OptiX remain gates |
