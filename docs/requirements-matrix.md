...
| Cycles | canonical loader, MetalRT and OptiX device integration | pinned source build and same-scene device comparison | native Cycles branch `0b4e499570f3` builds a tet-cage AABB/query path with object visibility filtering, mixed triangle/AABB candidates, projected-axis tet intersection, projected-axis local multi-hit, static volume query dispatch, motion-time local-hit normals, motion-aware native local scans, native AABB volume-table hits, compiled self-filtered opaque-shadow seams, the motion payload handoff, cancellation/error propagation, and the local self-hit filter; Blender branch `d92d347fdc75` builds the full app, recognizes `cycles_tetcage_v1`, keeps object-transform motion on static tet vertices while interpolating true geometry motion, and passes opt-in tet-only, mixed ordinary-triangle/native-AABB, one- and two-object object-transform motion, static transparent, pure-transparent motion, two-object shape-key deformation and closed-tet motion-local SSS, AO-local, UV, source-normal, two-material, one native shape-key deformation, static and one-/two-object object-transform volume-table probes, and local-table differentials; mixed transparent-closure motion remains an explicit ordinary-Cycles fallback and is contract-tested; area-light precision is numeric-close for point-like footprints but open for the default large footprint even at higher samples/shadow-disabled control, as contract-tested; open single-triangle SSS ordering, broader deformation topology, exact volume/closure semantics, Principled shading semantics, M9, and NVIDIA/OptiX remain gates |
