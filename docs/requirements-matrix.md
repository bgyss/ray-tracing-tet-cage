...
| Hit provenance | source primitive/material plus source barycentrics | position, UV, normal, material and custom-attribute differential | position, normal, UV, shader-visible source-normal corner attribute, and source material-slot IDs implemented and probed; full renderer material/custom-attribute parity pending |
...
| Cycles | canonical loader, MetalRT and OptiX device integration | pinned source build and same-scene device comparison | native Cycles branch `3723557ae33a` builds a tet-cage AABB/query path with object visibility filtering, mixed triangle/AABB candidates, projected-axis tet intersection, projected-axis local hit recording, native AABB volume-table hits, native AABB local multi-hit hits, compiled self-filtered opaque-shadow seams, the motion payload handoff, cancellation/error propagation, and the local self-hit filter; Blender branch `cf324b80705f` builds the full app, recognizes `cycles_tetcage_v1`, and passes opt-in tet-only, mixed ordinary-triangle/native-AABB, object-transform-motion, transparent, AO-local, UV, source-normal, two-material, one native shape-key deformation, volume-table, and local-table differentials; broader deformation topology, SSS area exactness/record-all ordering, Principled shading semantics, M9, and NVIDIA/OptiX remain gates |
