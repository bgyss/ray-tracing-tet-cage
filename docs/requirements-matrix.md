...
| Cycles | canonical loader, MetalRT and OptiX device integration | pinned source build and same-scene device comparison | native Cycles branch `44f2b9f9cc50` builds a tet-cage AABB/query path with object visibility filtering, mixed triangle/AABB candidates, compiled self-filtered opaque-shadow/local/volume seams, the motion payload handoff, and the local self-hit filter; Blender branch `3b24eb4f0606` builds the full app, recognizes `cycles_tetcage_v1`, and passes opt-in tet-only, mixed ordinary-triangle/native-AABB, tet-only and mixed object-transform-motion, transparent, and AO-local differentials; closed-tet volume is measured with a numeric difference, while deformation motion, SSS/record-all/local-runtime exactness, Principled shading semantics, M9, and NVIDIA/OptiX remain gates |
