...
| Cycles | canonical loader, MetalRT and OptiX device integration | pinned source build and same-scene device comparison | static native Cycles branch `bb9508937` builds a tet-cage AABB BLAS/query path with object visibility filtering, a compiled self-filtered opaque-shadow seam, source provenance packed into `ShaderData`, and an emission reference match on Apple M1 Max; Blender candidate branch `6c3b190fd0a` builds embedded scene/device/CPU-kernel/bridge targets plus the full Blender app, direct importer probe, and Cycles/UI smoke, recognizing `cycles_tetcage_v1` while retaining ordinary mesh fallback; motion, record-all/local/volume/transparent/full shading semantics, native Blender dispatch, M9, and NVIDIA/OptiX remain gates |
