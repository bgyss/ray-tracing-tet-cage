...
| Fast-path crack policy | versioned expanded clipping tolerance, later scale/ULP/conditioning policy | Metal/Vulkan boundary ray corpus with miss/duplicate rates | measured on M1 Max: fallback now supplies final hits/attributes and leaves zero final misses/ownership errors, but handles 1,099/1,408 corpus rays; pure hardware records 672 classified mismatches, so method-selection acceptance remains open |
...
| Metal fast backend | immutable micro-BLAS, GPU transforms/descriptors, TLAS, tracing | M5 correctness and scale sweep | measured partial: four-asset corpus, eight-frame dense animation, six TLAS refits, one rebuild, zero dense CPU mesh regenerations, and standard/extended 65,536-instance builds are direct; all 672 reductions preserve classification under real Metal replay and every merged final hit validates, but the fallback's 78.1% corpus frequency keeps M5 method-selection acceptance open |
