...
| Cycles | canonical loader, MetalRT and OptiX device integration | pinned source build and same-scene device comparison | static native Cycles branch `68ecd6550c6d` builds a tet-cage AABB/query path with object visibility filtering, mixed triangle/AABB candidates, a compiled self-filtered opaque-shadow seam, and a compiled local triangle scan; Blender branch `d3da8bef39b4` builds the full app, recognizes `cycles_tetcage_v1`, and passes opt-in tet-only and mixed ordinary-triangle/native-AABB emission differentials; motion, record-all/local-runtime/volume/transparent/Principled shading semantics, M9, and NVIDIA/OptiX remain gates |
