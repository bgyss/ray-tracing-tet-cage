...
| Fast-path crack policy | versioned expanded clipping tolerance, later scale/ULP/conditioning policy | Metal/Vulkan boundary ray corpus with miss/duplicate rates | M1 Max worktree result: procedural AABB traversal plus compensated projected-edge acceptance passes all 1,408 CPU-audited rays with zero hardware mismatches, misses, wrong owners, or CPU final hits; paired GPU-only runs invoke no oracle and emit all 1,408 records; clean rerun and Vulkan coverage remain |
...
| Metal fast backend | immutable micro-BLAS, GPU transforms/descriptors, TLAS, tracing | M5 correctness and scale sweep | direct worktree correctness is now 1,408/1,408 Metal final hits with zero CPU fallback and zero audited mismatch; explicit GPU-only mode constructs/invokes no CPU oracle; prior dense refit/rebuild and 65,536-instance evidence used the older kernel, so clean evidence and repeated procedural-kernel performance/content coverage remain |
