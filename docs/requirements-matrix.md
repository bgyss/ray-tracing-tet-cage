...
| Cycles | canonical loader, MetalRT and OptiX device integration | pinned source build and same-scene device comparison | native Cycles branch `2fefa15ad30f` builds a tet-cage AABB/query path with object visibility filtering, mixed triangle/AABB candidates, projected-axis tet intersection, projected-axis local multi-hit and motion-local hit recording, native AABB volume-table hits, compiled self-filtered opaque-shadow seams, the motion payload handoff, cancellation/error propagation, and the local self-hit filter; Blender branch `7add5059ffbf` builds the full app, recognizes `cycles_tetcage_v1`, and passes opt-in tet-only, mixed ordinary-triangle/native-AABB, object-transform-motion, transparent, AO-local, UV, source-normal, two-material, one native shape-key deformation, volume-table, and local-table differentials; animated SSS ordering, broader deformation topology, SSS area exactness, Principled shading semantics, M9, and NVIDIA/OptiX remain gates |
